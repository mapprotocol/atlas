...
func (evm *EVM) Call(caller vm.ContractRef, addr common.Address, input []byte, gas uint64, value *big.Int, fee *big.Int) (ret []byte, leftOverGas uint64, err error) {
	...
	if fee != nil {
		...
		if value.Sign() != 0 && !evm.Context.CanTransfer(evm.StateDB, caller.Address(), amount) {
			return nil, gas, errors.New("insufficient balance for transfer and fee ")
		}
	} else {
...
func (evm *EVM) create(caller vm.ContractRef, codeAndHash *codeAndHash, gas uint64, value *big.Int, address common.Address, fee *big.Int) ([]byte, common.Address, uint64, error) {
	...
	if fee != nil {
		...
		if !evm.Context.CanTransfer(evm.StateDB, caller.Address(), amount) {
			return nil, common.Address{}, gas, errors.New("insufficient balance for transfer and fee ")
		}
	} else {
		if !evm.Context.CanTransfer(evm.StateDB, caller.Address(), value) {
	...
	// above we revert to the snapshot and consume any gas remaining. Additionally
	// when we're in homestead this also counts for code storage gas errors.
	//if maxCodeSizeExceeded || (err != nil && (evm.ChainConfig().IsTIP3(evm.BlockNumber) || err != vm.ErrCodeStoreOutOfGas)) {
	//	evm.StateDB.RevertToSnapshot(snapshot)
	//	if err != vm.ErrExecutionReverted {
	//		contract.UseGas(contract.Gas)
	//	}
	//}
	if maxCodeSizeExceeded || (err != nil && (evm.chainRules.IsHomestead || err != vm.ErrCodeStoreOutOfGas)) {
		evm.StateDB.RevertToSnapshot(snapshot)
		if err != vm.ErrExecutionReverted {
			contract.UseGas(contract.Gas)
		}
