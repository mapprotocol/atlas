...
func (m *mockBackend) BlockChain() *chain.BlockChain {
	return m.bc
}
...
func createMiner(t *testing.T) (*Miner, *event.TypeMux) {
	...
	genesis := chain.DeveloperGenesisBlock(15, common.HexToAddress("12345"))
	chainConfig, _, err := chain.SetupGenesisBlock(chainDB, genesis)
	if err != nil {
	...
	bc, err := chain.NewBlockChain(chainDB, nil, chainConfig, engine, vm.Config{}, nil, nil)
	if err != nil {
