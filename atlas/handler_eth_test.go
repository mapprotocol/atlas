...
func (h *testEthHandler) Chain() *chain.BlockChain    { panic("no backing chain") }
...
func testForkIDSplit(t *testing.T, protocol uint) {
	...
	var (
		...
		gspecNoFork  = &chain.Genesis{Config: configNoFork}
		gspecProFork = &chain.Genesis{Config: configProFork}

		genesisNoFork  = gspecNoFork.MustCommit(dbNoFork)
		genesisProFork = gspecProFork.MustCommit(dbProFork)

		chainNoFork, _  = chain.NewBlockChain(dbNoFork, nil, configNoFork, engine, vm.Config{}, nil, nil)
		chainProFork, _ = chain.NewBlockChain(dbProFork, nil, configProFork, engine, vm.Config{}, nil, nil)

		blocksNoFork, _  = chain.GenerateChain(configNoFork, genesisNoFork, engine, dbNoFork, 2, nil)
		blocksProFork, _ = chain.GenerateChain(configProFork, genesisProFork, engine, dbProFork, 2, nil)
