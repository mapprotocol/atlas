...
func (p *peer) sendTransactions(txs types.Transactions) error {
	...
	return p2p.Send(p.rw, eth.TransactionsMsg, txs)
}
...
func (p *peer) sendPooledTransactionHashes(hashes []common.Hash) error {
	...
	return p2p.Send(p.rw, eth.NewPooledTransactionHashesMsg, hashes)
}
...
func (p *peer) SendPooledTransactionsRLP(hashes []common.Hash, txs []rlp.RawValue) error {
	...
	return p2p.Send(p.rw, eth.PooledTransactionsMsg, txs)
}
...
func (p *peer) SendNewBlockHashes(hashes []common.Hash, numbers []uint64) error {
	...
	return p2p.Send(p.rw, eth.NewBlockHashesMsg, request)
}
...
func (p *peer) SendNewBlock(block *types.Block, td *big.Int) error {
	...
	p.knownBlocks.Add(block.Hash())
	return p2p.Send(p.rw, eth.NewBlockMsg, []interface{}{block, td})
}
...
func (p *peer) SendBlockHeaders(headers []*types.Header) error {
	return p2p.Send(p.rw, eth.BlockHeadersMsg, headers)
}

// SendBlockBodiesRLP sends a batch of block contents to the remote peer from
// an already RLP encoded format.
func (p *peer) SendBlockBodiesRLP(bodies []rlp.RawValue) error {
	return p2p.Send(p.rw, eth.BlockBodiesMsg, bodies)
}

// SendNodeDataRLP sends a batch of arbitrary internal data, corresponding to the
// hashes requested.
func (p *peer) SendNodeData(data [][]byte) error {
	return p2p.Send(p.rw, eth.NodeDataMsg, data)
}

// SendReceiptsRLP sends a batch of transaction receipts, corresponding to the
// ones requested from an already RLP encoded format.
func (p *peer) SendReceiptsRLP(receipts []rlp.RawValue) error {
	return p2p.Send(p.rw, eth.ReceiptsMsg, receipts)
}
...
func (p *peer) RequestOneHeader(hash common.Hash) error {
	...
	return p2p.Send(p.rw, eth.GetBlockHeadersMsg, &getBlockHeadersData{Origin: eth.HashOrNumber{Hash: hash}, Amount: uint64(1), Skip: uint64(0), Reverse: false})
}
...
func (p *peer) RequestHeadersByHash(origin common.Hash, amount int, skip int, reverse bool) error {
	...
	return p2p.Send(p.rw, eth.GetBlockHeadersMsg, &getBlockHeadersData{Origin: eth.HashOrNumber{Hash: origin}, Amount: uint64(amount), Skip: uint64(skip), Reverse: reverse})
}
...
func (p *peer) RequestHeadersByNumber(origin uint64, amount int, skip int, reverse bool) error {
	...
	return p2p.Send(p.rw, eth.GetBlockHeadersMsg, &getBlockHeadersData{Origin: eth.HashOrNumber{Number: origin}, Amount: uint64(amount), Skip: uint64(skip), Reverse: reverse})
}
...
func (p *peer) RequestBodies(hashes []common.Hash) error {
	...
	return p2p.Send(p.rw, eth.GetBlockBodiesMsg, hashes)
}
...
func (p *peer) RequestNodeData(hashes []common.Hash) error {
	...
	return p2p.Send(p.rw, eth.GetNodeDataMsg, hashes)
}

// RequestReceipts fetches a batch of transaction receipts from a remote node.
func (p *peer) RequestReceipts(hashes []common.Hash) error {
	p.Log().Debug("Fetching batch of receipts", "count", len(hashes))
	return p2p.Send(p.rw, eth.GetReceiptsMsg, hashes)
}

// RequestTxs fetches a batch of transactions from a remote node.
func (p *peer) RequestTxs(hashes []common.Hash) error {
	p.Log().Debug("Fetching batch of transactions", "count", len(hashes))
	return p2p.Send(p.rw, eth.GetPooledTransactionsMsg, hashes)
}
...
func (p *peer) Handshake(network uint64, td *big.Int, head common.Hash, genesis common.Hash, forkID forkid.ID, forkFilter forkid.Filter) error {
	...
	go func() {
		...
		case p.version == istanbul.Celo64:
			errc <- p2p.Send(p.rw, eth.StatusMsg, &statusData63{
		...
		case p.version >= istanbul.Celo65:
			errc <- p2p.Send(p.rw, eth.StatusMsg, &statusData{
				ProtocolVersion: uint32(p.version),
				NetworkID:       network,
				TD:              td,
				Head:            head,
