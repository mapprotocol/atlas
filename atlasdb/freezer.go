...
func (f *freezer) freeze(db ethdb.KeyValueStore) {
	...
	for {
		...
		head := ReadHeader(nfdb, hash, m, *number)
		if head == nil {
		...
		for i := 0; i < len(ancients); i++ {
			...
			if first+uint64(i) != 0 {
				DeleteBlockWithoutNumber(batch, ancients[i], m, first+uint64(i))
				DeleteCanonicalHash(batch, m, first+uint64(i))
			}
		}
		...
		for number := first; number < f.frozen; number++ {
			// Always keep the genesis block in active database
			if number != 0 {
				dangling = ReadAllHashes(db, m, number)
				for _, hash := range dangling {
					log.Trace("Deleting side chain", "number", number, "hash", hash)
					DeleteBlock(batch, hash, m, number)
				}
			}
		}
		if err := batch.Write(); err != nil {
		...
		if f.frozen > 0 {
			...
			for len(dangling) > 0 {
				...
				children := ReadAllHashes(db, m, tip)
				for i := 0; i < len(children); i++ {
					// Dig up the child and ensure it's dangling
					child := ReadHeader(nfdb, children[i], m, tip)
					if child == nil {
					...
					DeleteBlock(batch, children[i], m, tip)
				}
				dangling = children
				tip++
			}
