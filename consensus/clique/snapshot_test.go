...
func TestClique(t *testing.T) {
	...
	for i, tt := range tests {
		...
		blocks, _ := chain.GenerateChain(&config, genesis.ToBlock(db), engine, db, len(tt.votes), func(j int, gen *chain.BlockGen) {
		...
		chain, err := chain.NewBlockChain(db, nil, &config, engine, vm.Config{}, nil, nil)
		if err != nil {
